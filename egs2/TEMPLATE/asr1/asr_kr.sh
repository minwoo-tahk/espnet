...
lm_text_sets=
score_original_text=false

help_message=$(cat << EOF
Usage: $0 --train_datasets "<train_dataset_names>" --test_datasets "<test_dataset_names>" --train-set "<train_set_name>" --valid-set "<valid_set_name>" --test_sets "<test_set_names>"
...
Options:
    ...
    --train_datasets         # The directories of training data set.
    --test_datasets          # The directories of test data set.
    --noisy_augmentation     # noisy data augmentation(default=false)
    --lm_text_sets           # add more text sets for lm training.
    --score_original_text    # score original text for korean(default=false)
...
[ -z "${train_datasets}" ] && { log "${help_message}"; log "Error: --train_datasets is required"; exit 2; };
[ -z "${test_datasets}" ] && { log "${help_message}"; log "Error: --test_datasets is required"; exit 2; };
[ -z "${train_set}" ] && { log "${help_message}"; log "Error: --train_set is required"; exit 2; };
[ -z "${valid_set}" ] && { log "${help_message}"; log "Error: --valid_set is required"; exit 2; };
[ -z "${test_sets}" ] && { log "${help_message}"; log "Error: --test_sets is required"; exit 2; };

train_combined="${train_set}_combined"

# Added by Minwoo
test_set="${test_sets}"
for dataset in ${test_datasets}; do
    dname=`echo ${dataset} | sed -e "s;\(.*\)/\(.*\);\2;i"`
    test_sets="${test_sets} $dname"
done
...
if [ -z "${lm_train_text}" ]; then
    if [ -n "${speed_perturb_factors}" ]; then
        lm_train_text="${data_feats}/${train_set}_sp/text"
    else
        lm_train_text="${data_feats}/${train_set}/text"
    fi
    
    if "${noisy_augmentation}"; then
        train_set_name=`echo ${lm_train_text} | sed -e "s;/\(.*\)/\(.*\)/\(.*\);\2;i"`
        lm_train_text=`echo ${lm_train_text} | sed -e "s;/${train_set_name}/;/${train_set_name}_noise/;"`
    fi
fi
# Use the same text as ASR for lm training if not specified.
...
echo -e "bpe_train_text: $bpe_train_text"
echo -e "lm_train_text: $lm_train_text"
echo -e "lm_dev_text: $lm_dev_text"
echo -e "lm_test_text: $lm_test_text"
...
echo -e "token_listdir: $token_listdir"
...
echo -e "bpedir: $bpedir"
echo -e "bpeprefix: $bpeprefix"
echo -e "bpemodel: $bpemodel"
echo -e "bpetoken_list: $bpetoken_list"
echo -e "chartoken_list: $chartoken_list"

# NOTE: keep for future development.
# shellcheck disable=SC2034
wordtoken_list="${token_listdir}"/word/tokens.txt

echo -e "wordtoken_list: $wordtoken_list"
...
echo -e "lm_token_list: $lm_token_list"
echo -e "lm_token_type: $lm_token_type"
...
echo -e "asr_stats_dir: $asr_stats_dir"
...
echo -e "lm_stats_dir: $lm_stats_dir"
...
echo -e "asr_exp: $asr_exp"
echo -e "lm_exp: $lm_exp"
...
echo -e "inference_tag: $inference_tag"
...
if ! "${skip_data_prep}"; then
    ...
    if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
        if [ -n "${speed_perturb_factors}" ]; then
            ...
            for factor in ${speed_perturb_factors}; do
                ...
                if "${noisy_augmentation}"; then
                    NOISY_SYNTH_ROOT=/data/users/asr_ws/tools/asr.noisyspeech_synthesizer
                    mkdir -p data/${train_set}_sp${factor}_noisy
                    scripts/utils/perturb_data_dir_speed.sh "${factor}" "data/${train_set}" "data/${train_set}_sp${factor}"
                    cat data/${train_set}_sp${factor}/wav.scp | \
                        sed -e "s;^sp${factor}-;sp${factor}-noisy-;" | \
                        sed -e "s;$; python ${NOISY_SYNTH_ROOT}/synthesizer.py |;" > data/${train_set}_sp${factor}_noisy/wav.scp
                    cat data/${train_set}_sp${factor}/text | \
                        sed -e "s;^sp${factor}-;sp${factor}-noisy-;" > data/${train_set}_sp${factor}_noisy/text
                    cat data/${train_set}_sp${factor}/utt2uniq | \
                        sed -e "s;^sp${factor}-;sp${factor}-noisy-;" > data/${train_set}_sp${factor}_noisy/utt2uniq
                    cat data/${train_set}_sp${factor}/utt2spk | \
                        sed -e "s;sp${factor}-;sp${factor}-noisy-;g" > data/${train_set}_sp${factor}_noisy/utt2spk
                    cat data/${train_set}_sp${factor}/spk2utt | \
                        sed -e "s;sp${factor}-;sp${factor}-noisy-;g" > data/${train_set}_sp${factor}_noisy/spk2utt
                    
                    _dirs+="data/${train_set}_sp${factor}_noisy "
                fi
            done

            if "${noisy_augmentation}"; then
                utils/combine_data.sh "data/${train_set}_sp_noise" ${_dirs}
            else
            ...
            if "${noisy_augmentation}"; then
                ...
                utils/combine_data.sh "data/${train_set}_noise" ${_dirs}
            fi
    ...
    if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
        ...
        # cat data/${train_combined}/text | awk ' { if( NF != 1 ) print $0; } ' > "${data_feats}/lm_train.txt"
    fi
...
if ! "${skip_train}"; then
    if "${use_lm}"; then
        ...
        if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ]; then
            rm -rf ${lm_exp}/perplexity_test/ppl

            for lm_text in ${lm_test_text} ${lm_dev_text}; do
            # for lm_text in ${lm_dev_text}; do
                log "Stage 8: Calc perplexity: ${lm_text}"
                _opts=

                dset=`echo ${lm_text} | sed -e "s;\(.*\)/\(.*\)/\(.*\);\2;i"`
                _dir="${lm_exp}/perplexity_test/${dset}"
                _log="${_dir}/lm_calc_perplexity.log"

                # TODO(kamo): Parallelize?
                log "Perplexity calculation started... log: '${_log}'"
                # shellcheck disable=SC2086
                ${cuda_cmd} --gpu "${ngpu}" "${_log}" \
                    ${python} -m espnet2.bin.lm_calc_perplexity \
                        --ngpu "${ngpu}" \
                        --data_path_and_name_and_type "${lm_text},text,text" \
                        --train_config "${lm_exp}"/config.yaml \
                        --model_file "${lm_exp}/${inference_lm}" \
                        --output_dir "${_dir}" \
                        ${_opts}
                log "PPL: ${lm_text}: $(cat ${_dir}/ppl)"
                echo -e "${lm_text}: $(cat ${_dir}/ppl)" >> ${lm_exp}/perplexity_test/ppl
            done
    ...
    if [ ${stage} -le 9 ] && [ ${stop_stage} -ge 9 ]; then
        ...
        # Added by Minwoo
        # [ ! -d "${expdir}" ] && ln -sf ${expdir}
        [ ! -d "$(pwd)/exp" ] && ln -sf ${expdir}
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        ...
        echo -e "_fold_length: ${_fold_length}"
        echo -e "asr_text_fold_length: ${asr_text_fold_length}"
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        for dset in ${valid_set} ${test_set}; do
    ...
    if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ]; then
        ...
        if "${score_original_text}"; then
            for dset in ${valid_set} ${test_set}; do
                _from_data="data/${dset}"
                _to_data="${data_feats}/${dset}"
                cat ${_from_data}/text.old > ${_to_data}/text.old
            done
        fi
        ...
        # for dset in ${valid_set} ${test_set}; do
            ...
            for _type in wer; do
                ...
                if "${score_original_text}"; then
                    sclite \
                        -r "${_scoredir}/ref.trn.old" trn \
                        -h "${_scoredir}/hyp.trn" trn \
                        -i rm -o all stdout > "${_scoredir}/result.txt.old"

                    log "Write ${_type} result in ${_scoredir}/result.txt.old"
                    grep -e Avg -e SPKR -m 2 "${_scoredir}/result.txt.old"
                fi
            done
        done
