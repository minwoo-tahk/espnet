...
if ! "${skip_data_prep}"; then
    if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
        ...
        # local/data_char.sh ${local_data_opts}
    fi
...
if ! "${skip_eval}"; then
    ...
    if [ ${stage} -le 13 ] && [ ${stop_stage} -ge 13 ]; then
        ...
        for dset in ${test_sets}; do
            ...
            # echo -e "lang: ${lang}"
            # echo -e "token_type: ${token_type}"

            # if [ "${lang}" = kr ] && [ "${token_type}" = char ]; then
            #     if [ ! -f "${_data}/text.jamo" ]; then
            #         mv -f ${_data}/text ${_data}/text.jamo
            #         mv -f data/test_data_01/text.old ${_data}/text
            #     fi

            #     if [ ! -f "${_dir}/text.jamo" ]; then
            #         mv -f ${_dir}/text ${_dir}/text.jamo
            #         python local/change_to_text.py -f ${_dir}/text.jamo
            #     fi
            # fi
            # # exit 0
